...
public class StdMOB implements MOB {
    ...
    protected CharStats                               baseCharStats    = (CharStats) CMClass
            .getCommon("DefaultCharStats");
    protected CharStats                               charStats        = (CharStats) CMClass
            .getCommon("DefaultCharStats");

    protected PhyStats                                phyStats         = (PhyStats) CMClass
            .getCommon("DefaultPhyStats");
    protected PhyStats                                basePhyStats     = (PhyStats) CMClass
            .getCommon("DefaultPhyStats");
    ...
    public CharState                                  curState         = (CharState) CMClass
            .getCommon("DefaultCharState");
    public CharState                                  maxState         = (CharState) CMClass
            .getCommon("DefaultCharState");
    public CharState                                  baseState        = (CharState) CMClass
            .getCommon("DefaultCharState");
    ...
    public int getExpNeededDelevel() {
        if (basePhyStats().level() <= 1)
            return 0;
        if ((CMSecurity.isDisabled(CMSecurity.DisFlag.EXPERIENCE)) || (charStats().getCurrentClass().expless())
    ...
    public void recoverCharStats() {
        baseCharStats.setClassLevel(baseCharStats.getCurrentClass(),
                basePhyStats().level() - baseCharStats().combinedSubLevels());
        ...
        {
            Log.errOut(
                    "Char Stat Out of Range: " + Name() + ": " + CMLib.map().getDescriptiveExtendedRoomID(location()));
            Log.errOut(x);
            if (isMonster()) {
    ...
    public void destroy() {
        ...
        if ((CMSecurity.isDebugging(CMSecurity.DbgFlag.MISSINGKIDS)) && (fetchEffect("Age") != null)
                && CMath.isInteger(fetchEffect("Age").text())
                && (CMath.s_long(fetchEffect("Age").text()) > Short.MAX_VALUE))
            Log.debugOut("MISSKIDS", new Exception(
                    Name() + " went missing form " + CMLib.map().getExtendedRoomID(CMLib.map().roomLocation(this))));
        if (soulMate() != null)
            dispossess(false);
    ...
    public void bringToLife(Room newLocation, boolean resetStats) {
        ...
        if ((miscText != null) && (resetStats) && (isGeneric())) {
            if (CMProps.getBoolVar(CMProps.Bool.MOBCOMPRESS) && (miscText instanceof byte[])) {
                CMLib.coffeeMaker().resetGenMOB(this, CMLib.coffeeMaker().getGenMOBTextUnpacked(this,
        ...
        if (location() == null) {
            Log.errOut("StdMOB",
                    name() + " of " + CMLib.map().getDescriptiveExtendedRoomID(newLocation) + " was auto-destroyed!");
            destroy();
            return;
        }
    ...
    protected boolean isPermissableToFight(final MOB mob) {
        ...
        if (this.isAttributeSet(MOB.Attrib.PLAYERKILL)) {
            if (CMSecurity.isAllowed(this, location(), CMSecurity.SecFlag.PKILL)
                    || (mob.isAttributeSet(MOB.Attrib.PLAYERKILL)))
                return true;
            return false;
        } else if (mob.isAttributeSet(MOB.Attrib.PLAYERKILL)) {
            if (CMSecurity.isAllowed(mob, location(), CMSecurity.SecFlag.PKILL)
                    || (this.isAttributeSet(MOB.Attrib.PLAYERKILL)))
                return true;
            return false;
        } else
            return false;
    ...
    public boolean mayPhysicallyAttack(MOB mob) {
        if ((!mayIFight(mob)) || (location() != mob.location()) || (!CMLib.flags().isInTheGame(this, false))
        ...
        if (mob != null) {
            if ((mob.location() == null) || (location() == null) || (mob.amDead()) || (amDead())
                ...
                if (Log.combatChannelOn()) {
                    ...
                    Log.combatOut("STRT",
                            Name() + ":" + phyStats().getCombatStats() + ":" + curState().getCombatStats() + ":"
                                    + ((I == null) ? "null" : I.name()) + ":" + mob.Name() + ":"
    ...
    public String displayText(MOB viewerMob) {
        if ((displayText.length() == 0) || (!name(viewerMob).equals(Name())) || (!titledName().equals(Name()))
                || (CMLib.flags().isSleeping(this)) || (CMLib.flags().isSitting(this)) || (riding() != null)
            ...
            } else if ((this instanceof Rideable) && (((Rideable) this).numRiders() > 0)
            ...
            synchronized (commandQue) {
                ...
                if (diff >= 0.0) {
                    final long nextTime = lastCommandTime
                            + Math.round(cmd.actionCost / phyStats().speed() * CMProps.getTickMillisD());
    ...
    public void enqueCommand(List<String> commands, int metaFlags, double actionCost) {
        if (commands == null)
            return;
        final CMObject O = CMLib.english().findCommand(this, commands);
        if ((O == null) || ((O instanceof Ability) && CMath.bset(metaFlags, MUDCmdProcessor.METAFLAG_ORDER)
    ...
    public boolean okMessage(final Environmental myHost, final CMMsg msg) {
            ...
            if (!msg.sourceMajor(CMMsg.MASK_ALWAYS)) {
                ...
                if (CMath.bset(srcCode, CMMsg.MASK_MOUTH)) {
                    ...
                    if (CMath.bset(srcCode, CMMsg.MASK_SOUND)) {
                        ...
                        if ((!CMLib.flags().canBeSeenBy(msg.target(), this)) && (!(isMine(msg.target())))
                ...
                if (CMath.bset(srcCode, CMMsg.MASK_HANDS)) {
                    ...
                    if ((CMLib.flags().isSitting(this)) && (msg.sourceMinor() != CMMsg.TYP_SITMOVE)
                            && (msg.sourceMinor() != CMMsg.TYP_BUY) && (msg.sourceMinor() != CMMsg.TYP_BID)
                            ...
                            && (((!CMLib.utensils().reachableItem(this, msg.target()))
                                    || (!CMLib.utensils().reachableItem(this, msg.tool()))) && (location() != null)
                                    && (!CMath.bset(location().phyStats().sensesMask(),
                ...
                if (CMath.bset(srcCode, CMMsg.MASK_MOVE)) {
                    ...
                    final boolean sitting = ((CMLib.flags().isSitting(this)) && (msg.sourceMinor() != CMMsg.TYP_LEAVE)
                            && (msg.sourceMinor() != CMMsg.TYP_ENTER));

                    if ((sleeping || sitting) && (msg.sourceMinor() != CMMsg.TYP_STAND)
                            && (msg.sourceMinor() != CMMsg.TYP_SITMOVE) && (msg.sourceMinor() != CMMsg.TYP_SIT)
                        ...
                        if ((msg.target() == null) || (!(msg.target() instanceof Deity))) {
                            if (msg.target() != null) {
                                if ((msg.target() instanceof MOB)
                                    ...
                                    return false;
                                } else if ((msg.target() instanceof MOB) && (((MOB) msg.target()).amFollowing() == srcM)
                                    ...
                                    tell(L("@x1 does not serve you, and you do not serve @x2.",
                                            ((Physical) msg.target()).name(this),
                                            ((Physical) msg.target()).name(this)));
                                    return false;
                                } else if ((msg.target() instanceof MOB)
                                        && (((MOB) msg.target()).getLiegeID().equals(Name()))
                                    ...
                                    tell(L("You cannot rebuke @x1.  You must get an annulment or a divorce.",
                                            ((Physical) msg.target()).name(this)));
                                    return false;
                                }
        ...
        if ((msg.sourceCode() != CMMsg.NO_EFFECT) && (msg.amISource(this)) && (msg.target() instanceof MOB)
                && (msg.target() != this) && (!CMath.bset(msg.sourceMajor(), CMMsg.MASK_ALWAYS)) && (location() != null)
            ...
            if (((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK) && (rangeToTarget() > maxRangeWith(msg.tool())))
                    || ((msg.sourceMinor() == CMMsg.TYP_THROW) && (rangeToTarget() > 2)
                ...
                if (victim != null) {
                    ...
                    setRangeToTarget(-1);
                recoverPhyStats();
            } else if ((msg.tool() != null) && (msg.sourceMinor() != CMMsg.TYP_BUY)
                    && (msg.sourceMinor() != CMMsg.TYP_BID) && (msg.sourceMinor() != CMMsg.TYP_SELL)
                ...
                if ((useRange >= 0) && (maxRangeWith(tool) < useRange)) {
                    ...
                    if ((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK) && (tool instanceof Weapon)
        ...
        if ((msg.targetMinor() != CMMsg.NO_EFFECT) && (msg.amITarget(this))) {
            ...
            if (CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS)) {
                ...
                if ((msg.amISource(this)) && (!msg.sourceMajor(CMMsg.MASK_ALWAYS)) && ((msg.tool() == null)
                ...
                if ((!mayIFight(srcM)) && ((!(msg.tool() instanceof Ability))
                        || (((((Ability) msg.tool()).classificationCode() & Ability.ALL_ACODES) != Ability.ACODE_POISON)
                                && ((((Ability) msg.tool()).classificationCode()
                                        & Ability.ALL_ACODES) != Ability.ACODE_DISEASE))
                ...
                if ((srcM != this) && (!isMonster()) && (!srcM.isMonster()) && (soulMate() == null)
                        && (srcM.soulMate() == null)
                        ...
                        && ((!(msg.tool() instanceof Ability)) || (((Ability) msg.tool()).classificationCode()
                                & Ability.ALL_ACODES) != Ability.ACODE_DISEASE)) {
                ...
                if (isInCombat()) {
                    if ((rangeToTarget() > 0) && (getVictim() != srcM) && (srcM.getVictim() == this)
            ...
            switch (msg.targetMinor()) {
                ...
                case CMMsg.TYP_TEACH:
                    if ((msg.target() instanceof MOB) && (!CMLib.expertises().canBeTaught(msg.source(),
                            (MOB) msg.target(), msg.tool(), msg.targetMessage())))
                        return false;
                    break;
                ...
                case CMMsg.TYP_GIVE:
                    ...
                    if (msg.target() instanceof MOB) {
                        ...
                        if (!location().okMessage(msg.target(), msg2)) {
                            srcM.tell(L("@x1 cannot seem to accept @x2.", ((Physical) msg.target()).name(srcM),
                                    ((Physical) msg.tool()).name(this)));
                            return false;
                        }
                    }
                    break;
                case CMMsg.TYP_FOLLOW:
                    ...
                    if ((CMProps.getIntVar(CMProps.Int.FOLLOWLEVELDIFF) > 0) && (!isMonster()) && (!srcM.isMonster())
                        ...
                        if (phyStats
                                .level() > (srcM.phyStats().level() + CMProps.getIntVar(CMProps.Int.FOLLOWLEVELDIFF))) {
        ...
        if ((srcM != this) && (msg.target() != this)) {
            if ((msg.othersMinor() == CMMsg.TYP_DEATH) && (msg.sourceMinor() == CMMsg.TYP_DEATH)) {
                if ((followers != null) && (followers.containsFirst(srcM)) && (CMLib.dice().rollPercentage() == 1)
    ...
    protected String fixChannelColors(final int channelCode, final String message) {
        ...
        if (session != null && session.getColorCodes()[128 + channelCode] != null)
            return CMStrings.replaceAll(message, ColorLibrary.SpecialColor.CHANNEL.getEscapeCode(),
                    mySession.getColorCodes()[128 + channelCode]);
        else {
    ...
    public void executeMsg(final Environmental myHost, final CMMsg msg) {
        ...
        final boolean canhearsrc = (msg.targetMinor() == CMMsg.TYP_SPEAK)
                ? CMLib.flags().canBeHeardSpeakingBy(srcM, this) : CMLib.flags().canBeHeardMovingBy(srcM, this);
        ...
        if (msg.amITarget(this) && (!amDead)) {
            switch (msg.targetMinor()) {
                ...
                case CMMsg.TYP_TEACH:
                    if (msg.target() instanceof MOB)
                        CMLib.expertises().handleBeingTaught(msg.source(), (MOB) msg.target(), msg.tool(),
                                msg.targetMessage());
                    break;
                default:
                    break;
        ...
        if ((msg.sourceCode() != CMMsg.NO_EFFECT) && (msg.amISource(this))) {
            if ((CMath.bset(msg.sourceMajor(), CMMsg.MASK_MALICIOUS)) && (msg.target() instanceof MOB)
                    && (getVictim() != msg.target()) && ((!CMath.bset(msg.sourceMajor(), CMMsg.MASK_ALWAYS))
                            || (!(msg.tool() instanceof DiseaseAffect)))) {
                CMLib.combat().establishRange(this, (MOB) msg.target(), msg.tool());
                if ((msg.tool() instanceof Weapon) || (msg.sourceMinor() == CMMsg.TYP_WEAPONATTACK)
            ...
            if (CMath.bset(msg.sourceMajor(), CMMsg.MASK_CHANNEL)) {
                ...
                switch (msg.sourceMinor()) {
                    ...
                    case CMMsg.TYP_DEATH:
                        ...
                        break;
                    case CMMsg.TYP_REBUKE:
                        if (((msg.target() == null) && (getLiegeID().length() > 0)) || ((msg.target() != null)
                    ...
                    case CMMsg.TYP_REMOVE:
                        ...
                        break;
                    default:
                        // you pretty much always know what you are doing, if
                        // you can do
                        // it.
            ...
            switch (msg.targetMinor()) {
                ...
                default:
                    if ((CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS)) && (!amDead))
                        ...
                        if ((playerStats() != null) && (!this.isAttributeSet(MOB.Attrib.QUIET))
            ...
            if ((CMath.bset(targetMajor, CMMsg.MASK_SOUND)) && (canhearsrc) && (!asleep)) {
                if ((msg.targetMinor() == CMMsg.TYP_SPEAK) && (srcM != null) && (playerStats() != null)
                        && (!srcM.isMonster()) && (CMLib.flags().canBeHeardSpeakingBy(srcM, this)))
                    playerStats().setReplyTo(srcM, PlayerStats.REPLY_SAY);

                tell(srcM, msg.target(), msg.tool(), msg.targetMessage());
            } else if ((CMath.bset(targetMajor, CMMsg.MASK_ALWAYS)) || (msg.targetMinor() == CMMsg.TYP_DAMAGE)
                ...
                tell(srcM, msg.target(), msg.tool(), msg.targetMessage());
            else if (((CMath.bset(targetMajor, CMMsg.MASK_HANDS)) || (CMath.bset(targetMajor, CMMsg.MASK_MOVE))
            ...
            if (CMath.bset(msg.othersMajor(), CMMsg.MASK_MALICIOUS) && (msg.target() instanceof MOB)
            ...
            if ((othersMinor == CMMsg.TYP_ENTER) // exceptions to movement
                ...
                if ((CMProps.getBoolVar(CMProps.Bool.INTRODUCTIONSYSTEM)) && (msg.othersMinor() == CMMsg.TYP_SPEAK))
                ...
                if (CMLib.flags().canSmell(this))
                    tell(srcM, msg.target(), msg.tool(), msg.othersMessage());
            } else if (((CMath.bset(othersMajor, CMMsg.MASK_EYES)) || (CMath.bset(othersMajor, CMMsg.MASK_HANDS))
                    || (CMath.bset(othersMajor, CMMsg.MASK_ALWAYS)))
                    && (!CMath.bset(msg.othersMajor(), CMMsg.MASK_CNTRLMSG)) && ((!asleep) && (canseesrc))) {
                tell(srcM, msg.target(), msg.tool(), msg.othersMessage());
            ...
            if ((othersMinor == CMMsg.TYP_DEATH) && (othersMinor == CMMsg.TYP_DEATH) && (location() != null))
    ...
    public boolean tick(final Tickable ticking, final int tickID) {
        ...
        if (tickID == Tickable.TICKID_MOB) {
            ...
            if (amDead) {
                    ...
                    if ((phyStats().rejuv() != PhyStats.NO_REJUV) && (basePhyStats().rejuv() > 0)) {
                        ...
                        if ((phyStats().rejuv() < 0) || (CMProps.getBoolVar(CMProps.Bool.MUDSHUTTINGDOWN))) {
                            ...
                            if (room != null) {
                                ...
                                if ((lastTickedTime < 0) && room.getMobility()
                ...
                if ((lastTickedTime < 0) && isMonster && R.getMobility() && (A.getAreaState() != Area.State.FROZEN)
                        && (A.getAreaState() != Area.State.STOPPED)) {
                    if (lastTickedTime == -1)
                        lastTickedTime = CMLib.utensils().processVariableEquipment(this);
                    else
                ...
                if ((CMProps.getIntVar(CMProps.Int.COMBATSYSTEM) == CombatLibrary.CombatSystem.TURNBASED.ordinal())
                    ...
                    setActions(actions()
                            + (CMLib.flags().isSitting(this) ? phyStats().speed() / 2.0 : phyStats().speed()));
                }
                ...
                if (!CMLib.flags().isGolem(this)) {
                    if (!CMLib.flags().canBreathe(this)) {
                        ...
                        R.show(this, this, CMMsg.MSG_OK_VISUAL,
                                L("^Z<S-NAME> can't breathe!^.^?") + CMLib.protocol().msp("choke.wav", 10));
                        ...
                        if ((atmo & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_LIQUID) {
                            final MOB killerM = CMLib.combat().getBreatheKiller(this);
                            R.show(this, this, CMMsg.MSG_OK_VISUAL,
                                    L("^Z<S-NAME> <S-IS-ARE> drowning in @x1!^.^?",
                                            RawMaterial.CODES.NAME(atmo).toLowerCase())
                                            + CMLib.protocol().msp("choke.wav", 10));
                            CMLib.combat().postDamage(killerM, this, null,
                                    (int) Math.round(CMath.mul(Math.random(), basePhyStats().level() + 2)),
                                    CMMsg.MASK_ALWAYS | CMMsg.TYP_WATER, -1, null);
                        } else {
                            if (atmo == 0)
                                ...
                                R.show(this, this, CMMsg.MSG_OK_VISUAL,
                                        L("^Z<S-NAME> <S-IS-ARE> choking on @x1!^.^?",
                                                RawMaterial.CODES.NAME(atmo).toLowerCase())
                                                + CMLib.protocol().msp("choke.wav", 10));
                            final MOB killerM = CMLib.combat().getBreatheKiller(this);
                            CMLib.combat().postDamage(killerM, this, null,
                ...
                if (isInCombat()) {
                    if (CMProps.getIntVar(CMProps.Int.COMBATSYSTEM) == CombatLibrary.CombatSystem.DEFAULT.ordinal())
                        setActions(actions() + 1.0); // bonus action is employed
                                                     // in default system
                    ...
                    if (this.isAttributeSet(MOB.Attrib.AUTODRAW) && (peaceTime >= START_SHEATH_TIME)
                ...
                if ((!isMonster) && (maxState().getFatigue() > Long.MIN_VALUE / 2)) {
                    ...
                    else // rest/sit isn't here because fatigue is sleepiness,
                         // not exhaustion per se
                ...
                if ((!isMonster) && (soulMate() == null)) {
                    ...
                    if (((++tickAgeCounter) * CMProps.getTickMillis()) >= AGE_MILLIS_THRESHOLD) {
                        ...
                        CMLib.achievements().possiblyBumpAchievement(this, AchievementLibrary.Event.TIMEPLAYED,
                                (int) secondsPassed, this);
                        tickAgeCounter = 0;
                        if (inventory != null)
                            inventory.trimToSize();
                        if (affects != null)
    ...
    public Item fetchFromInventory(Item goodLocation, String itemName, Filterer<Environmental> filter,
    ...
    public boolean willFollowOrdersOf(MOB mob) {
        if ((amFollowing() == mob) || ((isMonster() && CMSecurity.isAllowed(mob, location(), CMSecurity.SecFlag.ORDER)))
                || (getLiegeID().equals(mob.Name()))
                || (CMLib.law().doesOwnThisProperty(mob, CMLib.map().getStartRoom(this))))
            return true;
        if ((!isMonster()) && (CMSecurity.isAllowedEverywhere(mob, CMSecurity.SecFlag.ORDER))
    ...
    public Enumeration<Ability> allAbilities() {
        final MultiListEnumeration<Ability> multi = new MultiListEnumeration<Ability>(
                new List[]
        { abilitys, charStats().getMyRace().racialAbilities(this) });
    ...
    public void setClan(String clanID, int role) {
        ...
        if (role < 0) {
            ...
            if (p == null) {
                ...
                if (C == null)
                    Log.errOut("StdMOB", "Unknown clan: " + clanID + " on " + Name() + " in "
                            + CMLib.map().getDescriptiveExtendedRoomID(location()));
                else {
                    ...
                    if ((isPlayer()) && CMSecurity.isDebugging(CMSecurity.DbgFlag.CLANMEMBERS))
                        Log.debugOut(
                                "User '" + Name() + "' had membership in '" + clanID + "' added with role " + role);
                }
            } else {
                if (p.second.intValue() != role) {
                    if ((isPlayer()) && CMSecurity.isDebugging(CMSecurity.DbgFlag.CLANMEMBERS))
                        Log.debugOut("User '" + Name() + "' had membership in '" + clanID + "' changed from "
                                + p.second.toString() + " to role " + role);
    ...
    public int[][] getAbilityUsageCache(final String abilityID) {
        ...
        if ((phyStats().level() != abilityUseTrig[0]) || (charStats().getCurrentClassLevel() != abilityUseTrig[1])
            ...
            if (thisItem.amWearingAt(wornCode)) {
                if (thisItem instanceof Armor) {
                    ...
                    if (lay >= (aboveOrAroundLayer - 1)) {
                        if (((lay > aboveOrAroundLayer - 2) && (lay < aboveOrAroundLayer + 2) && ((!equalOk)
    ...
    public Item fetchHeldItem() {
        ...
        if (heldRef != null) {
            ...
            if (I == null)
                return null;
            if ((I.owner() == this) && (I.amWearingAt(Wearable.WORN_HELD)) && (!I.amDestroyed())
                    && (I.container() == null))
                return I;
            possHeldItem = null;
        }
    ...
    public void moveItemTo(Item container) {
        ...
        if (CMLib.flags().isHidden(container))
            container.basePhyStats().setDisposition(
                    container.basePhyStats().disposition() & ((int) PhyStats.ALLMASK - PhyStats.IS_HIDDEN));
        ...
        if (container.owner() != null) {
            ...
            else if (container.owner() instanceof MOB)
